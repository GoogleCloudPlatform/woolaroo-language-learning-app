...
import { WORD_SCROLL_LIST_CONFIG } from '../translation-selector/word-scroll-list';
...
export class CarouselComponent implements AfterViewInit, OnDestroy {
  ...
  stopDrag() {
    ...
    if (this.isDragging) {
      ...
      this.dragInfo = null;
      if(wordIndex >= 0 && this.items && wordIndex < this.items.length) {
  ...
  updateDrag(x: number, y: number) {
    ...
    this.currentItem = this.getWordIndexFromScrollPosition(scrollPosition, this.currentItem);
    this.updateTargetPosition();
  }
  ...
  private updateTargetPosition() {
    ...
    const wordIndex = Math.min(this.currentItem, items.length - 2);
    if (wordIndex < 0) {
      return;
    }
    const containerBounds = scrollContainer.getBoundingClientRect();
  ...
  private getEndingSnapWordIndex(position: number, velocity: number): number {
    ...
    return this.getWordIndexFromScrollPosition(finalPosition, this.currentItem);
  }
  ...
  private updateSnapPosition(lastProperties: {position: number, velocity: number, time: number}, snapWordIndex: number) {
    ...
    if (Math.sign(dx) === Math.sign(snapDx) && Math.abs(dx) >= Math.abs(snapDx) && Math.abs(dx) < 10) {
      ...
      clearInterval(this.snapInterval);
      this.snapInterval = null;
      this.currentItem = this.getWordIndexFromScrollPosition(snapPosition);
    } else {
      ...
      lastProperties.time = t;
      this.currentItem = this.getWordIndexFromScrollPosition(position);
    }
    this.updateTargetPosition();
  }
